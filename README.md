...
| **Remote access** | Turns RDP (Remote Desktop) and SSH (OpenSSH Server) on or off with one click, including the service, registry, and firewall rules. Opens custom inbound ports. Access can be limited to the local network. Shows ready-made `mstsc` / `ssh` connect commands. |
...
| `rdp on` / `rdp off` / `rdp status` | Enable, disable, or check Remote Desktop (append `local` or `any` to set scope) |
| `ssh on` / `ssh off` / `ssh status` | Enable, disable, or check OpenSSH Server (installs it if missing) |
| `open port <p> [tcp\|udp]` / `close port <p> [tcp\|udp]` | Open or close an inbound port (e.g. `open port 8080`) |
...
- **RDP.** Sets `fDenyTSConnections = 0`, keeps Network Level Authentication (NLA) on, and adds `FWDASH-RDP-TCP/UDP` allow rules on the configured RDP port (read from the registry, default 3389). The built-in `RemoteDesktop-*` rules are disabled so the chosen scope actually applies. Windows Home editions have no RDP server, so the button is disabled there.
- **SSH.** If OpenSSH Server is missing, it is installed in the background with `Add-WindowsCapability` (1–5 minutes, needs Windows Update). Then `sshd` is set to Automatic and started, and a `FWDASH-SSH-TCP` rule is added on the port from `sshd_config` (default 22). The built-in `OpenSSH-Server-In-TCP` rule is disabled for the same scope reason.
- **Scope.** *Local network only* uses `RemoteAddress LocalSubnet` and is the default. *All addresses* uses `Any` and shows a warning first.
...
- Exposing RDP or SSH to the internet (scope *All addresses* plus router port forwarding) invites brute-force attacks. Prefer a VPN such as WireGuard or Tailscale, and use strong passwords or SSH keys.
- A `block port` rule takes priority over an `open port` rule on the same port.
...
| **Masofaviy kirish** | RDP (Remote Desktop) va SSH'ni (OpenSSH Server) bir tugma bilan yoqadi yoki o'chiradi: xizmat, reestr va firewall qoidalari birga sozlanadi. Istalgan kiruvchi portni ochadi. Kirishni faqat lokal tarmoq bilan cheklash mumkin. Tayyor `mstsc` / `ssh` ulanish buyruqlarini ko'rsatadi. |
...
| `rdp on` / `rdp off` / `rdp status` | Remote Desktop'ni yoqish, o'chirish yoki holatini ko'rish (oxiriga `local` yoki `any` qo'shsa bo'ladi) |
| `ssh on` / `ssh off` / `ssh status` | OpenSSH Server'ni yoqish, o'chirish yoki holatini ko'rish (yo'q bo'lsa o'rnatadi) |
| `open port <p> [tcp\|udp]` / `close port <p> [tcp\|udp]` | Kiruvchi portni ochish yoki yopish (m: `open port 8080`) |
...
- **RDP.** `fDenyTSConnections = 0` qilinadi, Network Level Authentication (NLA) yoqilgan holda qoladi va RDP porti uchun `FWDASH-RDP-TCP/UDP` ruxsat qoidalari qo'shiladi. Port reestrdan o'qiladi, standart 3389. Tanlangan cheklov haqiqatan ishlashi uchun Windows'ning o'z `RemoteDesktop-*` qoidalari o'chiriladi. Windows Home'da RDP server yo'q, shuning uchun u yerda tugma faol emas.
- **SSH.** OpenSSH Server o'rnatilmagan bo'lsa, u fonda `Add-WindowsCapability` orqali o'rnatiladi (1–5 daqiqa, Windows Update kerak). So'ng `sshd` Automatic qilib ishga tushiriladi va `sshd_config` dagi port uchun `FWDASH-SSH-TCP` qoidasi qo'shiladi (standart 22). Xuddi shu sabab bilan Windows'ning `OpenSSH-Server-In-TCP` qoidasi o'chiriladi.
- **Cheklov (scope).** *Faqat lokal tarmoq* `RemoteAddress LocalSubnet` ni ishlatadi va standart tanlov shu. *Hamma manzillar* `Any` ni ishlatadi va avval ogohlantirish chiqaradi.
...
- RDP yoki SSH'ni internetga ochish (*Hamma manzillar* va routerda port forwarding) brute-force hujumlarga yo'l ochadi. VPN (WireGuard, Tailscale) afzalroq, kuchli parol yoki SSH kalitlaridan foydalaning.
- Bitta port uchun `block port` qoidasi `open port` qoidasidan ustun turadi.
